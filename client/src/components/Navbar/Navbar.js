  ...
  <div id="topOfPage">
    <Responsive minWidth={Responsive.onlyTablet.minWidth}>
      <Menu borderless pointing>
        ...
        <Menu.Item key={3} onClick={props.openContactUsModal}>
          {" "}
          <h3>{"Contact Us"}</h3>
        </Menu.Item>
        <Menu.Item key={4} onClick={props.openMapModal}>
          {" "}
          <h3>{"Life of Trash"}</h3>
        </Menu.Item>
        ...
        <Menu.Menu position="right">
          {rightMenuItems.map((item, index) => (
            ...
            >
              <h3>{item.word}</h3>
              <Icon name={item.iconName} size="big" className="navbarIcons" />
            </Menu.Item>
          ))}
        </Menu.Menu>
      </Menu>
    ...
    <Responsive {...Responsive.onlyMobile}>
      <Menu borderless>
        ...
        <Menu.Item position="right" onClick={props.toggleMenu}>
          <Icon
            name="sidebar"
            position="right"
            size="large"
            className="navbarIconsHam"
          />
        </Menu.Item>
      </Menu>
    </Responsive>
